...
const zh: Dict = {
  ...
  'users.roleProtected': '不能管理高于或等于自己权限的角色',
  'users.dlgNew': '新建用户',
  'users.fldEmail': '邮箱',
  ...
  'pd.expand': '展开',
  'pd.collapse': '收起',
  'pd.channelCount': '{count} 个通道',
  'pd.currentTag': '当前使用',
  'pd.setCurrent': '设为当前',
  'pd.currentVersion': '当前使用',
  'pd.archive': '归档',
  'pd.restore': '恢复',
  'pd.noVersions': '暂无版本。',
  ...
  'pd.versionPick': '选择已有版本号',
  'pd.versionNew': '+ 新建版本号…',
  'pd.versionBack': '← 选择已有版本号',
  'pd.noTokens': '暂无 API 令牌。',
  'pd.revoked': '已吊销',
  'pd.tokenActive': '有效',
  'pd.confirmRevoke': '吊销令牌「{name}」?',
  'pd.confirmDeleteToken': '删除令牌「{name}」？删除后无法恢复。',
  'pd.copyUnavailable': '此令牌在启用“随时复制”之前签发，重新签发后即可复制。',
  ...
  'pd.startUpload': '开始上传',
  'pd.uploadConflictTitle': '同版本和通道已存在',
  'pd.uploadConflictBody': '{version} / {channel} 已有文件 {existing}。当前选择 {current}，请选择覆盖已有文件，或返回修改版本号/通道后重新上传。',
  'pd.overwriteUpload': '覆盖已有文件',
  'pd.renameUpload': '重新改名',
  ...
  'pd.publicTag': '已公开',
  'pd.dlgPublic': '公开访问',
  'pd.publicDesc': '生成一个无需令牌的公开下载链接。任何人扫码或打开即可直接下载本版本（永远不能上传）。',
  'pd.publicEnable': '开启公开访问',
  'pd.publicDisable': '关闭公开访问',
  'pd.publicLink': '公开下载链接',
  'pd.publicScan': '扫码直接下载',
  'pd.copyLink': '复制链接',
  'pd.openLink': '打开',
  ...
  'mem.grant': '+ 授予成员',
  'mem.rolesHint':
    '成员角色:客户管理员(管理整个客户/项目)、开发者(上传 + 下载)、查看者(仅下载)。全局管理员可管理全部，观察者仅可查看和下载。',
  'mem.colUser': '用户',
  'mem.colScope': '范围',
  'mem.colTarget': '目标',
  'mem.colRole': '角色',
...
const en: Dict = {
  ...
  'users.roleProtected': 'Cannot manage roles at or above your own level',
  ...
  'pd.expand': 'Expand',
  'pd.collapse': 'Collapse',
  'pd.channelCount': '{count} channels',
  'pd.currentTag': 'Current',
  'pd.setCurrent': 'Set current',
  'pd.currentVersion': 'Current',
  'pd.archive': 'Archive',
  'pd.restore': 'Restore',
  ...
  'pd.versionPick': 'Pick existing version',
  'pd.versionNew': '+ New version…',
  'pd.versionBack': '← Pick existing version',
  'pd.noTokens': 'No API tokens yet.',
  'pd.revoked': 'revoked',
  'pd.tokenActive': 'active',
  'pd.confirmRevoke': 'Revoke token "{name}"?',
  'pd.confirmDeleteToken': 'Delete token "{name}"? This cannot be undone.',
  'pd.copyUnavailable': 'Issued before copy-anytime; re-issue the token to enable copying.',
  ...
  'pd.uploadConflictTitle': 'Same version and channel already exists',
  'pd.uploadConflictBody':
    '{version} / {channel} already has {existing}. Current file is {current}; overwrite it, or rename the version/channel and upload again.',
  'pd.overwriteUpload': 'Overwrite existing',
  'pd.renameUpload': 'Rename instead',
  ...
  'pd.public': 'Public',
  'pd.publicTag': 'public',
  'pd.dlgPublic': 'Public access',
  'pd.publicDesc':
    'Create a token-less public download link. Anyone can scan or open it to download this version directly (upload is never allowed).',
  'pd.publicEnable': 'Enable public access',
  'pd.publicDisable': 'Disable public access',
  'pd.publicLink': 'Public download link',
  'pd.publicScan': 'Scan to download',
  'pd.copyLink': 'Copy link',
  'pd.openLink': 'Open',
  ...
  'mem.rolesHint':
    'Member roles: customer_admin (manage all of customer/project), developer (upload + download), viewer (download only). Global admins manage everything; observers can only view and download.',
  'mem.colUser': 'User',
  'mem.colScope': 'Scope',
