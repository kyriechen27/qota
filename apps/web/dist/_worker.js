...
var badRequest = (msg, code = "bad_request") => new HttpError(400, code, msg);
var unauthorized = (msg = "unauthorized") => new HttpError(401, "unauthorized", msg);
var forbidden = (msg = "forbidden") => new HttpError(403, "forbidden", msg);
var notFound = (msg = "not found") => new HttpError(404, "not_found", msg);
var conflict = (msg) => new HttpError(409, "conflict", msg);

// apps/worker/src/utils/encoding.ts
...
// apps/worker/src/utils/password.ts
var ITERATIONS = 1e5;
var KEYLEN = 32;
var SALT_LEN = 16;
...
async function hashPassword(password) {
  ...
  const hash2 = await pbkdf2(password, salt, ITERATIONS);
  return `pbkdf2$${ITERATIONS}$${base64UrlEncode(salt)}$${base64UrlEncode(hash2)}`;
}
...
async function verifyJwt(token, secret) {
  ...
  if (typeof exp === "number" && exp < Math.floor(Date.now() / 1e3)) return null;
  return payload;
}
...
async function audit(c, ev) {
  let actorType;
  let actorId = null;
  if (ev.actorType !== void 0) {
...
authRoutes.patch("/profile", requireUser, async (c) => {
  ...
  if (body.email !== void 0) {
    ...
    const dupe = await c.env.DB.prepare("SELECT id FROM users WHERE email = ? AND id <> ?").bind(email, u.id).first();
  ...
  const row = await c.env.DB.prepare(selectSql).bind(u.id).first();
  return c.json(dto(row));
});
authRoutes.post("/change-password", requireUser, async (c) => {
  ...
  const row = await c.env.DB.prepare("SELECT password_hash FROM users WHERE id = ?").bind(u.id).first();
  if (!row) throw unauthorized();
  const ok = await verifyPassword(body.oldPassword, row.password_hash);
  if (!ok) throw unauthorized("invalid credentials");
  const hash2 = await hashPassword(body.newPassword);
  await c.env.DB.prepare("UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?").bind(hash2, Date.now(), u.id).run();
...
// apps/worker/src/lib/memberships.ts
var ROLE_GRANTS = {
  customer_admin: /* @__PURE__ */ new Set([
    "view",
    "download",
    "upload",
    "manage_versions",
...
async function effectiveRoleOnCustomer(db, userId, customerId) {
  const row = await db.prepare("SELECT role FROM memberships WHERE user_id = ? AND customer_id = ?").bind(userId, customerId).first();
  return row?.role ?? null;
}
async function effectiveRoleOnProject(db, userId, projectId) {
  const proj = await db.prepare("SELECT role FROM project_memberships WHERE user_id = ? AND project_id = ?").bind(userId, projectId).first();
...
async function requireProjectAccess(db, user, projectId, action) {
  const row = await db.prepare("SELECT id, customer_id, code, name, default_channel FROM projects WHERE id = ?").bind(projectId).first();
...
async function requireCustomerAccess(db, user, customerId, action) {
  const row = await db.prepare("SELECT id, code, name FROM customers WHERE id = ?").bind(customerId).first();
...
function slugify(input) {
  ...
  return romanized.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48);
}
async function generateUniqueCode(name, fallbackPrefix, isTaken) {
...
async function customerCodeTaken(db, code) {
  const row = await db.prepare("SELECT id FROM customers WHERE code = ?").bind(code).first();
  return !!row;
}
async function projectCodeTaken(db, customerId, code) {
  const row = await db.prepare("SELECT id FROM projects WHERE customer_id = ? AND code = ?").bind(customerId, code).first();
  return !!row;
}

// apps/worker/src/routes/customers.ts
...
var CODE_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/;
customerRoutes.get("/", async (c) => {
  ...
  return c.json((rows.results ?? []).map(dto3));
});
...
projectRoutes.get("/", async (c) => {
  ...
  const customerId = customerIdQ ? Number(customerIdQ) : void 0;
...
projectRoutes.post("/", async (c) => {
  ...
  let code;
  if (body.code) {
    if (!CODE_RE2.test(body.code)) throw badRequest("code must match [a-z0-9_-]");
...
membershipRoutes.post("/", async (c) => {
  ...
  if (body.customerId) {
    ...
    const dup2 = await c.env.DB.prepare(
      "SELECT id FROM memberships WHERE user_id = ? AND customer_id = ?"
    ).bind(body.userId, body.customerId).first();
    if (dup2) throw conflict("membership already exists; use PATCH to change role");
    const r2 = await c.env.DB.prepare(
      `INSERT INTO memberships (user_id, customer_id, role, created_by, created_at) VALUES (?, ?, ?, ?, ?)`
    ).bind(body.userId, body.customerId, role, me.id, now).run();
    const id2 = Number(r2.meta.last_row_id);
    ...
    return c.json(dtoCust(row2), 201);
  }
  ...
  ).bind(body.userId, projectId, role, me.id, now).run();
  const id = Number(r.meta.last_row_id);
  const row = await c.env.DB.prepare(
...
membershipRoutes.patch("/:id", async (c) => {
  ...
  if (scope === "customer") {
    ...
    if (!row2) throw notFound();
    await requireCustomerAccess(c.env.DB, me, row2.customer_id, "manage_members");
    ...
    await audit(c, {
      ...
      targetType: "membership",
      targetId: id,
      meta: { from: row2.role, to: body.role, scope }
    });
    const updated2 = await c.env.DB.prepare(
      "SELECT id, user_id, customer_id, role, created_by, created_at FROM memberships WHERE id = ?"
    ).bind(id).first();
    return c.json(dtoCust(updated2));
  }
  const row = await c.env.DB.prepare(
    "SELECT id, user_id, project_id, role, created_by, created_at FROM project_memberships WHERE id = ?"
  ).bind(id).first();
...
membershipRoutes.delete("/:id", async (c) => {
  ...
  const me = c.get("user");
  if (scope === "customer") {
    const row2 = await c.env.DB.prepare("SELECT customer_id FROM memberships WHERE id = ?").bind(id).first();
    if (!row2) throw notFound();
    await requireCustomerAccess(c.env.DB, me, row2.customer_id, "manage_members");
  ...
  const row = await c.env.DB.prepare("SELECT project_id FROM project_memberships WHERE id = ?").bind(id).first();
  if (!row) throw notFound();
  const proj = await requireProjectAccess(c.env.DB, me, row.project_id, "manage_members");
...
function makeS3(env) {
  ...
  if (missing.length) {
    throw new HttpError(
      ...
      `\u5BF9\u8C61\u5B58\u50A8\u672A\u914D\u7F6E\uFF1A\u7F3A\u5C11 ${missing.join("\u3001")}\u3002Cloudflare/wrangler \u8FD0\u884C\u65F6\u6CA1\u6709\u672C\u5730\u78C1\u76D8\uFF0C\u5FC5\u987B\u914D\u7F6E R2/S3 \u51ED\u636E\uFF1B\u82E5\u662F\u672C\u5730\u6216\u81EA\u6258\u7BA1\uFF0C\u6539\u7528 Node \u8FD0\u884C\u65F6\uFF08npm run dev:node / docker compose\uFF09\uFF0C\u9ED8\u8BA4\u5373\u7528\u672C\u5730\u6587\u4EF6\u5939\u5B58\u50A8\u3002`
    );
  }
  ...
  return {
    ...
    async signPartUrl(key, uploadId, partNumber, ttlSeconds) {
      const url = `${signUrl(key)}?partNumber=${partNumber}&uploadId=${encodeURIComponent(uploadId)}&X-Amz-Expires=${ttlSeconds}`;
    ...
    async completeMultipartUpload(key, uploadId, parts) {
      ...
      const body = `<?xml version="1.0" encoding="UTF-8"?><CompleteMultipartUpload xmlns="http://s3.amazonaws.com/doc/2006-03-01/">` + sorted.map(
...
// apps/worker/src/routes/versions.ts
var versionRoutes = new Hono2();
versionRoutes.use("*", requireUser);
function versionDto(r) {
  return {
    id: r.id,
    projectId: r.project_id,
    version: r.version,
...
apiTokenRoutes.post("/:id/revoke", async (c) => {
  ...
  if (!Number.isFinite(id)) throw badRequest("invalid id");
  const me = c.get("user");
  const row = await c.env.DB.prepare("SELECT project_id FROM api_tokens WHERE id = ?").bind(id).first();
  if (!row) throw notFound();
  const proj = await requireProjectAccess(c.env.DB, me, row.project_id, "manage_tokens");
...
async function resolveApiToken(c, rawToken) {
  const hash2 = await sha256Hex(rawToken);
  const row = await c.env.DB.prepare(
  ...
  c.executionCtx.waitUntil(
    c.env.DB.prepare("UPDATE api_tokens SET last_used_at = ?, last_used_ip = ? WHERE id = ?").bind(Date.now(), ip, row.id).run()
  );
  return {
    id: row.id,
    projectId: row.project_id,
...
// apps/worker/src/routes/download.ts
...
async function redirectOrJson(c, row, ttl) {
  const s3 = makeStorage(c.env);
  const url = await s3.signGetUrl(row.r2_key, ttl, {
    filename: row.filename,
    contentType: row.content_type ?? void 0
...
var MAX_PART_SIZE = 64 * 1024 * 1024;
var MAX_PARTS = 1e4;
function clampPartSize(hint, totalSize) {
...
uploadRoutes.post("/init", async (c) => {
  ...
  if (!CHANNEL_RE2.test(channel)) throw badRequest("releaseChannel invalid");
...
uploadRoutes.post("/sign-part", async (c) => {
  ...
  return c.json({ url, expiresAt: Math.floor(Date.now() / 1e3) + ttl });
});
uploadRoutes.post("/complete", async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare("UPDATE upload_sessions SET status = ? WHERE id = ?").bind("failed", session.id).run();
    await audit(c, {
  ...
  for (const p of body.parts) {
    await c.env.DB.prepare(
      `INSERT OR REPLACE INTO upload_parts (session_id, part_number, etag, size, uploaded_at)
       VALUES (?, ?, ?, ?, ?)`
    ).bind(session.id, p.partNumber, p.etag, p.size, now).run();
  }
...
uploadRoutes.post("/abort", async (c) => {
  ...
  if (session.status === "in_progress") {
    ...
    await c.env.DB.prepare("UPDATE upload_sessions SET status = ? WHERE id = ?").bind("aborted", session.id).run();
  }
  await audit(c, {
...
uploadRoutes.get("/sessions/:id", async (c) => {
  ...
  if (!Number.isFinite(id)) throw badRequest("invalid id");
  const me = c.get("user");
  const row = await c.env.DB.prepare(`SELECT * FROM upload_sessions WHERE id = ?`).bind(id).first();
  if (!row) throw notFound();
...
auditRoutes.get("/", async (c) => {
  ...
  return c.json((rows.results ?? []).map(dto6));
});

// apps/worker/src/lib/bootstrap.ts
var seeded = false;
async function ensureAdminSeed(env) {
  ...
  try {
    ...
    ).bind(email, hash2, displayName, now, now).run();
    seeded = true;
...
var init_default = `-- qota v2: multi-tenant role model + S3 multipart upload sessions + audit logs.
